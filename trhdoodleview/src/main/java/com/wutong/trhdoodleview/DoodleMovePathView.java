...
public class DoodleMovePathView extends DoodleView {
    ...
    public DoodleMovePathView(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }


    public DoodleMovePathView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (statue == Edite && (System.currentTimeMillis() - downTime > timeLimit)) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (statue == Edite) {
                    ...
                    AreaData p = new AreaData(event.getX(), event.getY(), event.getX(), event.getY());
                    for (DrawDate v : drawDateList) {
                        if (v.isSelected) {
                            Log.i(TAG, "onTouchEvent: ");
                            if (AreaData.isIntersect(p, v.areaData) && v.isSelected) {
                                statue = Select;
                                firstX = event.getX();
                                firstY = event.getY();
                                rx = 0;
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (statue == Select) {
                    rx = event.getX() - firstX;
                    ry = event.getY() - firstY;
                    break;
                }
